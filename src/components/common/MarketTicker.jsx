...
export default function MarketTicker({ items = [], loading = false }) {
  const ticks = items.length ? items : defaultTicks
  const row = [...ticks, ...ticks]

  return (
    <div className="bg-primary text-white overflow-hidden border-y border-white/10">
      <div className={`flex gap-10 py-3 whitespace-nowrap animate-ticker ${loading ? 'opacity-70' : ''}`}>
        {row.map((t, i) => (
          <div key={`${t.name}-${i}`} className="flex items-center gap-2 text-sm">
            ...
            {t.chg && (
              <span className={t.isRate ? 'text-white/50' : t.up ? 'text-accent' : 'text-red-400'}>
                {!t.isRate && (t.up ? '▲' : '▼')} {t.chg}
              </span>
            )}
          </div>
