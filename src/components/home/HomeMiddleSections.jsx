...
const homeGoals = [
  { name: 'Retirement Plan', target: '₹2.5 Cr', pct: 68, icon: '🌴', iconSrc: goalIconSrc('Retirement Plan'), color: 'bg-amber-100 text-amber-700' },
  { name: 'Home Purchase', target: '₹80 L', pct: 40, icon: '🏡', iconSrc: goalIconSrc('Dream House'), color: 'bg-blue-100 text-blue-700' },
  { name: 'Child Education', target: '₹25 L', pct: 30, icon: '🎓', iconSrc: goalIconSrc('Child Education'), color: 'bg-violet-100 text-violet-700' },
]
...
function MyGoalsCard({ dashboard, loading = false }) {
  const liveGoals = dashboard?.goals ?? []
  const isLoggedIn = dashboard?.isLoggedIn
  const goals = liveGoals.length
    ? liveGoals
    : (!isLoggedIn ? homeGoals : [])
    ...
    <SectionCard className="p-6 h-full flex flex-col bg-gradient-to-br from-white to-emerald-50/20">
      ...
      {!loading && goals.length === 0 && (
        <div className="flex-1 flex flex-col items-center justify-center text-center py-6">
          <p className="text-sm text-slate-500 mb-3">No goals yet. Set a goal and invest toward it.</p>
          <Link
            to="/goals"
            className="text-xs font-bold text-[#0056D2] hover:text-[#003FA3]"
          >
            Add a goal →
          </Link>
        </div>
        ...
        {goals.map((g) => (
          <div key={g.id || g.name}>
            <div className="flex items-center gap-3 mb-3">
              <span className={`w-10 h-10 rounded-xl grid place-items-center overflow-hidden shrink-0 ${g.color || 'bg-slate-100'} shadow-md`}>
                {(g.iconSrc || goalIconSrc(g.goalType || g.name)) ? (
                  <img
                    src={g.iconSrc || goalIconSrc(g.goalType || g.name)}
                    alt={g.name}
            ...
            <div className="h-2.5 bg-slate-100 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${Math.min(100, Number(g.pct) || 0)}%` }}
              />
            </div>
          </div>
        ))}
...
function ContentThumb({ gradient, image, title }) {
  if (image) {
    return (
      <img
        src={image}
        alt={title || ''}
        className="w-20 h-16 rounded-xl shrink-0 object-cover shadow-md bg-slate-100"
        loading="lazy"
      />
    )
  }
...
function NewsBlogSection() {
  const newsQuery = useArticleList('news')
  const blogQuery = useArticleList('blog')
  ...
  const newsItems = (newsQuery.items ?? []).slice(0, 3)
  const blogItems = (blogQuery.items ?? []).slice(0, 3)
    ...
    <section className="grid lg:grid-cols-3 gap-6">
      <SectionCard className="p-6">
        ...
        <div className="space-y-4">
          {newsQuery.loading && (
            <p className="text-sm text-slate-500 py-6 text-center">Loading news…</p>
          )}
          {!newsQuery.loading && newsQuery.error && (
            <p className="text-sm text-red-600 py-4 text-center">{newsQuery.error}</p>
          )}
          {!newsQuery.loading && !newsQuery.error && newsItems.length === 0 && (
            <p className="text-sm text-slate-500 py-6 text-center">No news published yet.</p>
          )}
          {newsItems.map((n, i) => (
            <Link
              key={n.id || n.title}
              to={`/news/${n.id}`}
              className="flex gap-4 group hover:bg-slate-50 p-3 -mx-3 rounded-lg transition-colors"
            >
              <ContentThumb gradient={newsThumbs[i % newsThumbs.length]} image={n.image} title={n.title} />
      ...
      <SectionCard className="p-6">
        ...
        <div className="space-y-4">
          {blogQuery.loading && (
            <p className="text-sm text-slate-500 py-6 text-center">Loading blogs…</p>
          )}
          {!blogQuery.loading && blogQuery.error && (
            <p className="text-sm text-red-600 py-4 text-center">{blogQuery.error}</p>
          )}
          {!blogQuery.loading && !blogQuery.error && blogItems.length === 0 && (
            <p className="text-sm text-slate-500 py-6 text-center">No blogs published yet.</p>
          )}
          {blogItems.map((p, i) => (
            <Link
              key={p.id || p.title}
              to={`/blog/${p.id}`}
              className="flex gap-4 group hover:bg-emerald-50 p-3 -mx-3 rounded-lg transition-colors"
            >
              <ContentThumb gradient={blogThumbs[i % blogThumbs.length]} image={p.image} title={p.title} />
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
