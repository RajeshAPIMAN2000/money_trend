...
import { goalIconSrc, TRANSUNION_CIBIL_ICON } from '../lib/brandAssets.js'
...
/** Image icons keyed by product id / title where assets exist */
const PRODUCT_IMAGE_ICONS = {
  'cibil-score': TRANSUNION_CIBIL_ICON,
  'goal-planning': goalIconSrc('Goal Planning'),
}

function ProductIcon({ id, title, className }) {
  const imageSrc = PRODUCT_IMAGE_ICONS[id] || goalIconSrc(title)
  if (imageSrc) {
    return (
      <img
        src={imageSrc}
...
function ProductCard({ product, onCheckCibil }) {
  ...
  return (
    <article className={`group relative bg-white rounded-2xl border border-slate-100 p-6 shadow-[0_2px_12px_rgba(15,23,42,0.06)] hover:shadow-[0_12px_40px_rgba(15,23,42,0.12)] hover:-translate-y-1 transition-all duration-300 ${product.featured ? 'ring-2 ring-[#0056D2]/20' : ''}`}>
      ...
      <div className={`w-14 h-14 rounded-2xl ${product.iconBg} ${product.iconColor} grid place-items-center mb-5 group-hover:scale-105 transition-transform overflow-hidden p-1.5`}>
        <ProductIcon id={product.id} title={product.title} className="w-full h-full" />
      </div>
